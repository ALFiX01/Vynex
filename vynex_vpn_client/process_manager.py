...
from .constants import SINGBOX_EXECUTABLE, XRAY_EXECUTABLE, XRAY_RUNTIME_DIR
...
class SingboxProcessManager:
    ...
    def start(self, config: dict[str, Any]) -> int:
        if not SINGBOX_EXECUTABLE.exists():
            raise FileNotFoundError("sing-box.exe не найден.")
        self.ensure_no_running_instances()
        config_path = self._write_temp_config(config)
        self._output_tail.clear()
        creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
        self._process = subprocess.Popen(
            [str(SINGBOX_EXECUTABLE), "run", "-c", str(config_path)],
            cwd=str(XRAY_RUNTIME_DIR),
            creationflags=creationflags,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
    ...
    def stop(self, pid: int | None) -> None:
        ...
        if self._process and self._process.pid == pid and self._process.poll() is None:
            ...
            try:
                ...
                self._finalize_process()
                return
        if not is_process_running(pid) or not self._is_singbox_pid(pid):
            self._finalize_process()
            return
        creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
        subprocess.run(
    ...
    @staticmethod
    def is_running(pid: int | None) -> bool:
        return bool(pid and is_process_running(pid) and SingboxProcessManager._is_singbox_pid(pid))
    ...
    def list_running_instances(self) -> list[SingboxInstanceInfo]:
        creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
        result = subprocess.run(
            [
                "powershell",
        ...
        if len(lines) <= 1:
            return []
        instances: list[SingboxInstanceInfo] = []
        for row in csv.DictReader(lines):
            raw_pid = (row.get("ProcessId") or "").strip()
            if not raw_pid.isdigit():
                continue
    ...
    def _read_last_log_lines(self, limit: int = 15) -> str:
        tail = "\n".join(list(self._output_tail)[-limit:])
        return tail or "sing-box завершился сразу после запуска без вывода в лог."

    def read_recent_output(self, limit: int = 15) -> str:
        return self._read_last_log_lines(limit)

    @staticmethod
    def _is_singbox_pid(pid: int) -> bool:
        creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
        result = subprocess.run(
            ["tasklist", "/FI", f"PID eq {pid}", "/FO", "CSV", "/NH"],
            check=False,
        ...
        try:
            ...
            return False
        return bool(row and row[0].lower() == "sing-box.exe")
    ...
    @staticmethod
    def _format_running_instances_error(instances: list[SingboxInstanceInfo]) -> str:
        lines = ["Уже запущен sing-box.exe. Сначала остановите существующий экземпляр."]
        target_path = SingboxProcessManager._normalize_path(str(SINGBOX_EXECUTABLE))
        for instance in instances[:5]:
            location = instance.executable_path or "путь не определен"
            source = "управляемый клиентом" if instance.executable_path == target_path else "внешний"
            lines.append(f"PID {instance.pid} [{source}] - {location}")
