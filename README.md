...
**Современный VPN-клиент для Windows 10/11 с PySide6 GUI, быстрым импортом конфигов и управлением подключением через Xray, sing-box и AmneziaWG.**
...
Vynex — Windows-клиент для управления VPN-подключениями через графический интерфейс с импортом конфигов, подписок и быстрым запуском подключения.

В одном интерфейсе собраны основные сценарии работы с подключением:
...
3. Если runtime-компоненты отсутствуют, откройте экран `Компоненты` и обновите нужные файлы.
4. На экране `Сервера` добавьте share-link или используйте быстрый импорт для списка ссылок, `vpn://`, URL подписки или AWG-конфига.
5. Выберите сервер на экране `Подключение` или `Сервера` и нажмите `Подключиться`.
...
`python main.py` запускает основной PySide6 GUI. Для `TUN` при запуске из исходников используйте PowerShell от имени администратора и команду `python main.py`. Старый терминальный интерфейс оставлен как legacy fallback и запускается явно: `python main.py --terminal`.
...
Если хотите обновить компоненты вручную, это можно сделать из раздела `Компоненты` внутри GUI. Там же доступна проверка обновления приложения, открытие страницы релиза и self-update для packaged Windows `.exe`.

## Пример Использования

### Импорт одного сервера

На экране `Сервера` нажмите `Добавить` и вставьте share-link:
...
На экране `Подписки` нажмите `Добавить` и вставьте URL:
...
2. Выберите режим подключения и routing profile.
3. Для `TUN` перезапустите приложение от имени администратора.
4. На экране `Подключение` или `Сервера` выберите сервер и нажмите `Подключиться`.
...
Это не всегда означает, что подключение полностью не работает. В проекте health-check используется как диагностика: если runtime уже поднялся, но probe URL не ответили вовремя, клиент может оставить подключение активным и показать предупреждение вместо немедленного разрыва. Для `TUN` это относится к проверке после поднятия интерфейса, для `PROXY` — к случаям, когда локальные inbound уже слушают порт, но внешние probe URL только тайм-аутятся.
...
- `main.py` — основная точка входа PySide6 GUI.
- `vynex_vpn_client/gui/` — графический интерфейс PySide6.
- `vynex_vpn_client/app_service.py` — сервисный слой для GUI и будущих интерфейсов.
- `vynex_vpn_client/app.py` — legacy terminal UI, запускается через `python main.py --terminal`.
...
- ставит зависимости, включая PySide6, и `PyInstaller`;
- собирает GUI-приложение `dist\VynexVPNClient.exe` без консольного окна.
